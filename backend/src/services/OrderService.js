    ...
    try {
      ...
      for (const item of orderItems) {
        ...
        if (product.hasSizes) {
          ...
          if (!sizeEntry) {
            throw new Error(
              `Size "${item.size}" của sản phẩm "${item.name}" không tồn tại`
            );
          }
          if (sizeEntry.quantity < item.quantity) {
            // Lỗi hết hàng (nếu có người khác mua trong lúc đang checkout)
            throw new Error(
              `Sản phẩm "${item.name}" (Size: ${item.size}) không đủ số lượng. Chỉ còn ${sizeEntry.quantity}.`
            );
          }
          ...
          if (product.stock < item.quantity) {
            throw new Error(
              `Sản phẩm "${item.name}" không đủ số lượng. Chỉ còn ${product.stock}.`
            );
          }
          // Trừ kho tổng
        ...
        [
          {
            ...
            isPaid: isPaid || false, // <--- MỚI THÊM (Nếu ko có thì mặc định false)
            paidAt: paidAt || null,
          },
        ],
        { session }
