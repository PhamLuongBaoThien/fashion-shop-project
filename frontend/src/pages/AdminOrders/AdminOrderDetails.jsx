...
const AdminOrderDetails = () => {
  ...
  const mutation = useMutation({
    mutationFn: (data) => OrderService.updateOrder(id, user?.access_token, data),
    onSuccess: () => {
  ...
  const columns = [
    {
      ...
      render: (text, record) => (
        <Space>
          <img src={record.image} alt={text} style={{ width: 50, height: 50, objectFit: 'cover', borderRadius: 4 }} />
          <div>
             <div>{text}</div>
             <div style={{fontSize: 12, color: '#888'}}>Size: {record.size}</div>
          </div>
        </Space>
      )
    },
  ...
  return (
    <div style={{ padding: "0 24px 24px" }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
         ...
         <Button icon={<ArrowLeftOutlined />} onClick={() => navigate('/system/admin/orders')}>Quay lại</Button>
      </div>
      ...
      <Row gutter={24}>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
                <div style={{ width: 250 }}>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 18, fontWeight: 'bold' }}>
                        <Text>Tổng cộng:</Text>
                        <Text type="danger">{order.totalPrice?.toLocaleString()}đ</Text>
                    </div>
                </div>
            </div>
          </Card>
        ...
        <Col span={8}>
            <Card title="Cập nhật trạng thái" bordered={false} style={{ marginBottom: 24 }}>
                ...
                <div style={{ marginBottom: 24 }}>
                    ...
                    <div style={{ marginBottom: 12 }}>
                        <Tag color={statusColors[order.status || 'pending']} style={{ fontSize: 14, padding: '4px 10px' }}>
            ...
            <Card title="Thông tin Khách hàng" bordered={false}>
                 <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                     ...
                     <div>
                         <div style={{ fontWeight: 'bold' }}>{order.customerInfo?.fullName || "Khách vãng lai"}</div>
