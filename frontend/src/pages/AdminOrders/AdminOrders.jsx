...
const AdminOrders = () => {
    ...
    select: (data) => {
       ...
       if (search) {
         ...
         filtered = filtered.filter(order => 
           ...
           (order.customerInfo?.fullName && order.customerInfo.fullName.toLowerCase().includes(lowerSearch)) ||
  ...
  const handleSortChange = (value) => {
    setSearchParams({ sortOption: value, page: 1, limit, search, status: statusFilter });
  };
  ...
  const confirmDelete = () => {
      if (messageApi) messageApi.info("Tính năng xóa đơn hàng đang phát triển (API chưa sẵn sàng)");
  ...
  const handleExportExcel = () => {
    ...
    if (messageApi) messageApi.loading({ content: "Đang tạo file Excel...", key: "export" });
    ...
    try {
        ...
        const formattedData = ordersData.map(order => ({
            ...
            "Trạng thái": getOrderStatus(order) === "Delivered" ? "Đã giao" : 
                          getOrderStatus(order) === "Paid" ? "Đã thanh toán" : "Đang xử lý"
        }));
        ...
        if (messageApi) messageApi.success({ content: "Xuất file Excel thành công!", key: "export", duration: 2 });

    } catch (error) {
        console.error("Export error:", error);
        if (messageApi) messageApi.error({ content: "Xuất file thất bại!", key: "export", duration: 2 });
    }
  };
  ...
  const columns = [
    {
      ...
      dataIndex: "_id",
      key: "_id",
      render: (id) => <span style={{ fontFamily: 'monospace', fontWeight: 'bold' }}>#{id ? id.slice(-6).toUpperCase() : 'NA'}</span>,
    },
    {
      title: "Khách hàng",
      ...
      render: (customerInfo, record) => (
        <div style={{ display: 'flex', flexDirection: 'column' }}>
          <span style={{ fontWeight: 500 }}>{customerInfo?.fullName || "Guest"}</span>
          <span style={{ fontSize: '12px', color: '#888' }}>{customerInfo?.phone}</span>
          
          {record.shippingInfo?.fullName !== customerInfo?.fullName && (
             <Tooltip title={`Người nhận: ${record.shippingInfo?.fullName} - ${record.shippingInfo?.phone}`}>
                <Tag color="purple" style={{marginTop: 4, width: 'fit-content', fontSize: 10, cursor: 'help'}} icon={<UserOutlined />}>
                    Mua tặng
                </Tag>
             </Tooltip>
          )}
    ...
    },
    {
      ...
      render: (date) => (
          <div style={{ fontSize: '13px' }}>
            ...
            <div style={{ color: '#999' }}>{date ? new Date(date).toLocaleTimeString("vi-VN") : ""}</div>
          </div>
      ),
    },
    {
    ...
    },
    {
      ...
      key: "payment",
      render: (method) => (
          <Tag color={method === "cod" ? "cyan" : "blue"}>{method === "cod" ? "COD" : "Banking"}</Tag>
      )
    },
    {
      title: "Hành động",
      ...
      render: (_, record) => (
        <Space>
          <Link to={`/system/admin/orders/detail/${record._id}`}>
             <ButtonComponent type="primary" size="small" icon={<EyeOutlined />} />
          </Link>
  ...
  if (isError) {
      return <Alert message="Lỗi" description={error?.message || "Không thể tải danh sách đơn hàng"} type="error" showIcon />
  }
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
      ...
      >
         ...
         <p style={{fontSize: '12px', color: '#888'}}>Lưu ý: Hiện tại chỉ là giao diện, chức năng xóa chưa được kích hoạt.</p>
      </Modal>
    </motion.div>
  );
};
