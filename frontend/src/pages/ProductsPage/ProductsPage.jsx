...
const ProductsPage = () => {
  ...
  const params = {
    ...
    priceRange: searchParams.get('priceRange')?.split(',').map(Number) || [0, 5000000],
        ...
        <Row gutter={[20, 20]}>
          <Col xs={24} sm={24} md={6} lg={5} xl={4} className="sidebar-col">
            <FilterSidebar
              ...
              setSelectedCategories={(value) => handleFilterChange('category', value)}
          ...
          <Col xs={24} sm={24} md={18} lg={19} xl={20} className="product-list-col">
            {isLoading && !productsData ? ( // Chỉ hiển thị Spin khi tải lần đầu
              <div style={{ display: 'flex', justifyContent: 'center', paddingTop: '100px' }}><Spin size="large" /></div>
            ) : (
              <ProductsComponent
                products={products}
                viewMode={params.viewMode}
